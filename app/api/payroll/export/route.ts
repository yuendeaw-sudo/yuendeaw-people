...
export async function GET(req: Request) {
  ...
  const header = ["รหัส", "ชื่อ-นามสกุล", "ประเภท", "สถานะ", "เงินเดือน", "โบนัสเดือนนี้", "สวัสดิการเดือนนี้", "รวม"];
  const lines = [
    header.map(csvCell).join(","),
    ...rows.map((r) =>
      [r.code, r.name, r.type, r.status, r.salary, r.bonus, r.welfare, r.net].map(csvCell).join(",")
    ),
